...
Les noms des zones géographiques, bien qu'ils soient assez simples à mettre dans un outil de collecte de données, nécessitent souvent un certain travail de nettoyage dans le traitement des données pour que les résultats correspondent aux fichiers SIG.  Il est très fréquent qu'il y ait des problèmes et des confusions concernant l'orthographe, les translittérations, les noms alternatifs et les doublons des zones.
...
À cette fin, ce guide recommande d'utiliser les géocodes et les noms des ensembles de données opérationnelles communes (COD) de l'OCHA : 
...
Les données géographiques pour l'Afrique occidentale et centrale sont disponibles à l'adresse suivante  https://data.humdata.org/dataset/west-and-central-africa-administrative-boundaries-levels ou les utilisateurs peuvent rechercher les informations appropriées sur le pays dans https://data.humdata.org/dashboards/cod

Nous recommandons d'utiliser les codes des colonnes *admin1Pcode* et *admin2Pcode* dans la colonne *nom* du formulaire xls.  Il est également recommandé d'utiliser les codes des colonnes *admin1RefName* et *admin2RefName* dans la colonne *label* du formulaire xls.
